...
// Fetch number of tasks assigned to this W2
$sql_tasks = "SELECT COUNT(*) AS total FROM tasks WHERE assigned_to = $user_id";
$res_tasks = $conn->query($sql_tasks);
$total_tasks = ($res_tasks && $res_tasks->num_rows > 0) ? $res_tasks->fetch_assoc()['total'] : 0;

// Fetch number of completed tasks
$sql_completed = "SELECT COUNT(*) AS completed FROM completed_tasks WHERE employee_id = $user_id";
$res_completed = $conn->query($sql_completed);
$completed_tasks = ($res_completed && $res_completed->num_rows > 0) ? $res_completed->fetch_assoc()['completed'] : 0;
...
.container {
    ...
    display: flex;
    flex-direction: column;
    gap: 60px;                 /* reduce gap between rows */
    padding: 20px;
    margin-top: 70px;          /* karibu na header bar */
...
.card {
    ...
    text-align: center;
    width: 220px;          /* fix width ya kila card */
...
<div class="main" id="mainContent">
    ...
    <div class="card-row">
        <div class="card">
            ...
            <h3>Received Tasks</h3>
            <p><?php echo $total_tasks; ?> tasks assigned</p>
            <a href="received_tasks.php">View</a>
        </div>

        <div class="card">
            <div class="icon"><i class="fas fa-folder-open"></i></div>
            <h3>My Work</h3>
            <p><?php echo $completed_tasks; ?> tasks submitted</p>
            <a href="my_work.php">Manage</a>
        </div>
    ...
    <div class="card-row">
        <div class="card">
            ...
            <h3>Assign Task</h3>
            <p>Give tasks to employees</p>
            <a href="assign_task_w2.php">Assign</a>
        </div>

        <div class="card">
            <div class="icon"><i class="fas fa-check-double"></i></div>
            <h3>Approve / Rollback</h3>
            <p>Review submitted tasks</p>
            <a href="approve_tasks_w2.php">Review</a>
        </div>

        <div class="card">
            <div class="icon"><i class="fas fa-user-check"></i></div>
            <h3>Request Permission</h3>
            <p>Ask permission from W1</p>
            <a href="request_permission_w2.php">Request</a>
        </div>
    </div>
</div>
    </div>
...
document.getElementById("updateProfileForm").addEventListener("submit", function(e) {
    ...
    .then(data => {
        if (data.success) {
            ...
            document.querySelector('.header h1').innerHTML = `Welcome, ${data.username} (W2)`;
